			     ...
			     void *clientData __attribute__ ((unused)))
{
  ...
  new_map->complete_map = (float *)
    calloc(new_map->config.x_size*new_map->config.y_size, sizeof(float));
...
#endif 
  new_map->map = (float **)
    calloc(map_msg.config.x_size, sizeof(float *));
  ...
  if (client_map)
    {
      ...
      client_map->complete_map = (float *)
	calloc(client_map->config.x_size*client_map->config.y_size, 
	       sizeof(float));
...
#endif 
      client_map->map = (float **)
	calloc(response->config.x_size, sizeof(float *));
      carmen_test_alloc(client_map->map);
      for(i = 0; i < response->config.x_size; i++)
	client_map->map[i] = client_map->complete_map + 
	  i*response->config.y_size;	      
    }

...
static carmen_robot_laser_message front_laser, rear_laser;
static carmen_robot_sonar_message sonar;
static carmen_robot_vector_status_message vector;
static carmen_base_odometry_message odometry;

static int received_front_laser = 0;
static int received_rear_laser = 0;
static int received_sonar = 0;
static int new_front_laser = 0;
static int new_rear_laser = 0;
static int new_sonar = 0;
...
#endif
static void robot_frontlaser_handler(void) {
  received_front_laser = 1;
  new_front_laser = 1;
  redraw();
}

static void robot_rearlaser_handler(void) {
...
static void robot_sonar_handler(void) {
  received_sonar = 1;
  new_sonar = 1;
  redraw();
}

static void odometry_handler(void) {
  ...
  if (!rotating && !moving)
    carmen_robot_velocity_command(0.0, 0.0);
  else {
    if (!rotating)
      delta_angle = 0.0;
    if (!moving)
    ...
    //		 rotating, moving);
    carmen_robot_move_along_vector(displacement, delta_angle);
  }
}

static int initialize_robotgraph_ipc(void) {
  ...
  carmen_robot_subscribe_sonar_message(
      &sonar, (carmen_handler_t)robot_sonar_handler, CARMEN_SUBSCRIBE_LATEST);

  carmen_robot_subscribe_vector_status_message(&vector, NULL,
                                               CARMEN_SUBSCRIBE_LATEST);

  memset(&vector, 0, sizeof(carmen_robot_vector_status_message));
...
static void shutdown_robotgraph(int x) {
  if (x == SIGINT) {
#ifndef NO_JOYSTICK
...
static void save_postscript_screenshot(void) {
  ...
  carmen_ps_draw_line(
      doc, doc->width / 2.0, doc->height / 2.0,
      doc->width / 2.0 +
          length * 0.75 / 500.0 * 2.0 * cos(front_laser.robot_pose.theta),
      doc->height / 2.0 +
          length * 0.75 / 500.0 * 2.0 * sin(front_laser.robot_pose.theta));
  carmen_ps_close(doc);
  carmen_verbose("Generated %s\n", filename);
  screenshot_number++;
}

static void set_rotating(int x, int y) {
  ...
  // carmen_radians_to_degrees(destination_angle),
  // carmen_radians_to_degrees(delta_angle),
  // carmen_radians_to_degrees(odometry.theta));

  distance = hypot(delta_x, delta_y);

  if (distance < length * scale ||
      fabs(carmen_radians_to_degrees(destination_angle)) < 1.0) {
    destination_angle = 0;
    rotating = 0;
  } else {
    rotating = 1;
...
static void set_moving(int x, int y) {
  double delta_x, delta_y;
  double distance;
...
static gint key_press_event(GtkWidget *widget __attribute__((unused)),
  ...
  if (GDK_CONTROL_MASK & key->state || GDK_SHIFT_MASK & key->state ||
      GDK_MOD1_MASK & key->state || key->keyval > 255)
    return 1;
...
static void setup_colors(void) {
  ...
  gdk_gc_set_line_attributes(drawing_gc, 2, GDK_LINE_SOLID, GDK_CAP_NOT_LAST,
                             GDK_JOIN_MITER);
}

static void range_to_xy(carmen_robot_laser_message *laser, GdkPoint *laser_poly,
                        double *laser_dist) {
  int i;
  double theta;
  ...
  for (i = 0; i < laser->num_readings; i++) {
    ...
    laser_poly[i].x = width_2 +
                      ((laser->laser_pose.x - laser->robot_pose.x) +
                       cos(theta) * laser->range[i]) *
                          scale;
    laser_poly[i].y = height_2 -
                      ((laser->laser_pose.y - laser->robot_pose.y) +
                       sin(theta) * laser->range[i]) *
                          scale;
    if (i > 0)
      laser_dist[i] = hypot(laser_poly[i - 1].x - laser_poly[i].x,
                            laser_poly[i - 1].y - laser_poly[i].y);
  }
  laser_poly[laser->num_readings].x =
      width_2 + (laser->laser_pose.x - laser->robot_pose.x) * scale;
  laser_poly[laser->num_readings].y =
      height_2 - (laser->laser_pose.y - laser->robot_pose.y) * scale;
}

static void offset_polygon(GdkPoint *poly1, GdkPoint *poly2, int num_points,
...
static void check_capacity(int new_laser, int new_size, double **laser_dist,
  ...
  (*laser_dist) =
      (double *)realloc((*laser_dist), sizeof(double) * (new_size + 1));
  carmen_test_alloc(*laser_dist);

  (*laser_poly) =
      (GdkPoint *)realloc((*laser_poly), sizeof(GdkPoint) * (new_size + 1));

  carmen_test_alloc(*laser_poly);

  for (i = 0; i < MAX_SAVED_POLYGONS; i++) {
    free_polygons[i] = (GdkPoint *)realloc(free_polygons[i],
                                           sizeof(GdkPoint) * (new_size + 1));
...
static void update_history(int received_laser, carmen_point_t *last_odom,
                           carmen_point_t *current_odom, int *current_polygon,
                           GdkPoint *laser_poly, int poly_size,
                           GdkPoint *polygon_history[],
  ...
  if (hypot(current_odom->x - last_odom->x, current_odom->y - last_odom->y) >
          0.0 ||
      fabs(last_odom->theta - current_odom->theta) > 0.0) {
    memcpy(polygon_history[*current_polygon], laser_poly,
           sizeof(GdkPoint) * poly_size);
...
static void draw_sonar_arcs(GdkPixmap *pixmap) {
  int i;
  int x, y, w, h, theta1, theta2, radius;
  gdk_gc_set_foreground(drawing_gc, &carmen_green);
  //  printf("pose: %f %f %f   offset: %f %f %f\n", sonar.robot_pose.x,
  //  sonar.robot_pose.y, sonar.robot_pose.theta, sonar.sonar_offsets[0].x,
  //  sonar.sonar_offsets[0].y, sonar.sonar_offsets[0].theta);
  for (i = 0; i < sonar.num_sonars; i++) {
    radius = (int)(sonar.ranges[i] * scale);
    x = width_2 + scale * (sonar.sonar_offsets[i].x) - radius;
    y = height_2 - scale * (sonar.sonar_offsets[i].y) - radius;
    w = radius * 2;
    h = radius * 2;
    theta1 = carmen_radians_to_degrees(sonar.sonar_offsets[i].theta +
                                       sonar.robot_pose.theta -
                                       sonar.cone_angle / 2) *
             64;
...
static void draw_laser_endpoints(GdkPoint *laser_poly,
  ...
  for (i = 0; i < laser_msg->num_readings; i++) {
    if (laser_msg->tooclose[i] == 1)
      continue;
    if (i > 0 && laser_dist[i] < connect_distance)
      gdk_draw_line(pixmap, drawing_gc, laser_poly[i - 1].x,
                    laser_poly[i - 1].y, laser_poly[i].x, laser_poly[i].y);
    else
      gdk_draw_arc(pixmap, drawing_gc, TRUE, laser_poly[i].x - 2,
...
static void draw_robot_sonar(GdkPixmap *pixmap,
                             carmen_robot_sonar_message *sonar_msg) {
  ...
  if (!rectangular) {
    gdk_draw_arc(pixmap, drawing_gc, TRUE, width_2 - length / 2.0 * scale,
                 height_2 - length / 2.0 * scale, length * scale,
                 length * scale, 0, 360 * 64);
    if (draw_red)
      gdk_gc_set_foreground(drawing_gc, &carmen_red);
    else
  ...
  gdk_draw_line(
      pixmap, drawing_gc, width_2, height_2,
      width_2 + cos(sonar_msg->robot_pose.theta) * length / 2.0 * scale,
      height_2 - sin(sonar_msg->robot_pose.theta) * length / 2.0 * scale);
}

static void draw_robot(GdkPixmap *pixmap,
                       carmen_robot_laser_message *laser_msg) {
  GdkPoint rect_robot[4];
  GdkPoint p1, p2;
  GdkPoint top_left;
  ...
  gdk_gc_set_foreground(drawing_gc, &carmen_red);
  x_1 =
      width_2 + sin(laser_msg->robot_pose.theta) * laser_msg->turn_axis * scale;
  y_1 = height_2 +
        cos(laser_msg->robot_pose.theta) * laser_msg->turn_axis * scale;
  ...
  gdk_draw_line(
      pixmap, drawing_gc, width_2, height_2,
      width_2 + cos(laser_msg->robot_pose.theta) * length / 2.0 * scale,
      height_2 - sin(laser_msg->robot_pose.theta) * length / 2.0 * scale);
  if (show_velocity && fabs(laser_msg->tv) < 0.001 &&
      fabs(laser_msg->rv) > .00001) {
    gdk_gc_set_foreground(drawing_gc, &carmen_green);
    gdk_draw_arc(pixmap, drawing_gc, FALSE,
                 width_2 - length / 2.0 * scale * 1.1,
                 height_2 - length / 2.0 * scale * 1.1, length * scale * 1.1,
                 length * scale * 1.1, 0, 360 * 64);
  } else if (show_velocity && fabs(laser_msg->tv) > 0.001 &&
             fabs(laser_msg->rv) > 0.001) {
    gdk_gc_set_foreground(drawing_gc, &carmen_green);
    ...
    if (radius > 0) {
      top_left.x = width_2 +
                   cos(laser_msg->robot_pose.theta + M_PI / 2) *
                       (radius + width / 2.0) * scale -
                   radius * scale;
      top_left.y = height_2 -
                   sin(laser_msg->robot_pose.theta + M_PI / 2) *
                       (radius + width / 2.0) * scale -
                   radius * scale;
    ...
    p1.x =
        width_2 +
        cos(laser_msg->robot_pose.theta + M_PI / 2) * width / 2.0 * scale * 1.2;
    p1.y =
        height_2 -
        sin(laser_msg->robot_pose.theta + M_PI / 2) * width / 2.0 * scale * 1.2;
  ...
  if (show_vector && fabs(vector.vector_distance) > 0) {
    ...
    p2.x -= 5;
    p2.y -= 5;
    gdk_draw_arc(pixmap, drawing_gc, FALSE, p2.x, p2.y, 10, 10, 0, 360 * 64);
  }
}
...
static void redraw(void) {
  ...
  if (pixmap == NULL)
    pixmap =
        gdk_pixmap_new(drawing_area->window, drawing_area->allocation.width,
  ...
  if (!received_front_laser && !received_rear_laser && !received_sonar &&
      font != NULL) {
    gdk_gc_set_foreground(drawing_gc, &carmen_white);
    gdk_draw_string(pixmap, font, drawing_gc,
                    drawing_area->allocation.width / 2 - 50,
                    drawing_area->allocation.height / 2, "Waiting for data");
  }
  ...
  if (received_front_laser) {
    ...
    draw_laser_endpoints(front_laser_poly, &front_laser, front_laser_dist,
                         pixmap);
  ...
  if (received_rear_laser) {
    ...
    draw_laser_endpoints(rear_laser_poly, &rear_laser, rear_laser_dist, pixmap);

    new_rear_laser = 0;
  }
  if (received_sonar) {
    draw_sonar_arcs(pixmap);
    new_sonar = 0;
  }
...
#ifndef NO_JOYSTICK
static int Joystick_Event(GtkWidget *widget __attribute__((unused)),
  ...
  if (carmen_get_joystick_state(&joystick) >= 0 ||
    ...
    carmen_joystick_control(&joystick, max_tv, max_rv, &tv, &rv);
    carmen_robot_velocity_command(tv, rv);
...
static void read_robotgraph_parameters(int argc, char **argv) {
  int num_items;
  ...
  carmen_param_t param_list[] = {
      ...
      {"robotgui", "connect_distance", CARMEN_PARAM_DOUBLE, &connect_distance,
       1, NULL},
      {"robotgui", "gui_control", CARMEN_PARAM_ONOFF, &gui_control, 1, NULL},
      {"robotgui", "show_velocity", CARMEN_PARAM_ONOFF, &show_velocity, 1,
       NULL},
      {"robotgui", "show_vector", CARMEN_PARAM_ONOFF, &show_vector, 1, NULL}};
...
int main(int argc, char **argv) {
  ...
  while (!i) {
    ...
    if (i != 0) {
      ...
      err = carmen_keyboard_control(c, max_tv, max_rv, &tv, &rv);
      if (err < 0)
        shutdown_robotgraph(SIGINT);
      else {
        fprintf(stderr, "%c ", c);
